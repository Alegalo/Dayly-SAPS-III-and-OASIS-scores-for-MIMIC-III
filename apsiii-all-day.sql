...
(
  ...
  , case
      when  coalesce(FIO2, fio2_chartevents) is not null
        ...
        then ROW_NUMBER() over (partition by bg.ICUSTAY_ID ORDER BY CHARTTIME, AADO2 DESC)
      else null end
      as aado2_rn
  , case
      ...
      when vd.icustay_id is not null
          then null
      else ROW_NUMBER() over (partition by bg.ICUSTAY_ID ORDER BY CHARTTIME,PO2 DESC)
    end as pao2_rn
...
, acidbase_max as
(
  select icustay_id, acidbase_score, ph, paco2, charttime
    -- create integer which indexes maximum value of score with 1
  , case
      when ph is not null and paco2 is not null
        then ROW_NUMBER() over (partition by ICUSTAY_ID ORDER BY CHARTTIME, ACIDBASE_SCORE DESC)
      else null
    end as acidbase_rn
  from acidbase
)
, arf as
(
  select ie.subject_id, ie.icustay_id, ie.hadm_id, ie.intime, ie.outtime, uo.charttime_day as charttime
    , case
  ...
  where uo.charttime_day = labs.charttime
  group by 1,2,3,4,5,6,7
)
, cohort as
(
select arf.subject_id, arf.hadm_id, arf.icustay_id
      , arf.intime
      , arf.outtime
	  , case 
		     when vital.charttime is not null then vital.charttime
			 when bg.charttime is not null then bg.charttime
			 when labs.charttime is not null then labs.charttime
			 when uo.charttime_day is not null then uo.charttime_day
			 when gcs.charttime is not null then gcs.charttime
			 when vent.charttime is not null then vent.charttime
	         when arf.charttime is not null then arf.charttime 
	    end as charttime
      , vital.heartrate_min
      , vital.heartrate_max
      , vital.meanbp_min
      , vital.meanbp_max
      , CAST(vital.tempc_min as DECIMAL(12,10))
      , CAST(vital.tempc_max as DECIMAL(12,10))
      , vital.resprate_min
      , vital.resprate_max
...
left join acidbase_max ab
  on  bg.icustay_id = ab.icustay_id
  and bg.charttime = ab.charttime
left join arf
  on ab.icustay_id = arf.icustay_id
  and ab.charttime = arf.charttime
left join ventfirstday vent
  on arf.icustay_id = vent.icustay_id
  and arf.charttime = vent.charttime
left join gcsfirstday gcs
  on vent.icustay_id = gcs.icustay_id
  and gcs.charttime = vent.charttime
left join vitalsfirstday vital
  on gcs.icustay_id = vital.icustay_id
  and vital.charttime = gcs.charttime
left join uofirstday uo
  on vital.icustay_id = uo.icustay_id
  and uo.charttime_day = vital.charttime
left join labsfirstday labs
  on uo.icustay_id = labs.icustay_id
  and uo.charttime_day = labs.charttime	
)
, score_min as
(
  select cohort.subject_id, cohort.hadm_id, cohort.icustay_id, cohort.charttime
  , case
...
left join score_min smin
  ...
  and co.charttime = smin.charttime
left join score_max smax
  on co.icustay_id = smax.icustay_id
  and co.charttime = smax.charttime
)
, score as
(
  select s.*
...
select s.subject_id, s.hadm_id, s.icustay_id, s.charttime
, APSIII
...
, gcs_score
from score s
order by s.icustay_id, s.charttime;